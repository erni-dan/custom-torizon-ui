...
const requestTorizonAPI = async (endpoint, data = null, method = 'GET') => {
    switch (method) {
        case 'GET':
            if (data) 
            {
                return await torizon_api.get(endpoint + `/${data}`);
            }
