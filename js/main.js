...
function createRobot() {
    ...
    const grey_mesh =  new THREE.MeshNormalMaterial();
...
function createMyRobot() {
    ...
    const junction_geometry = new THREE.CylinderGeometry(0.6, 0.6, 0.7042, 32);
    const first_junction = new THREE.Mesh(junction_geometry, grey_mesh);
    first_junction.position.set(0.0, 1.2, 0);
    //first_junction.rotation.x = Math.PI/2;
    first_junction.castShadow = true;
    first_junction.receiveShadow = true;
...
function main() {
  ...
  var faktor = 1.0;
  var dummy_model_element = document.getElementById("dummy_model_title");
  console.log(dummy_model_element.offsetWidth, dummy_model_element.offsetHeight); //always return 0 
  console.log("fake it till you make it")
  console.log(dummy_model_element.offsetWidth, dummy_model_element.offsetWidth *faktor); 
  var dummy_model_size = dummy_model_element.getBoundingClientRect();
  console.log(dummy_model_size.right - dummy_model_size.left, dummy_model_size.bottom - dummy_model_size.top); 

  var scene = new THREE.Scene();
     // Camera
  const fov = 75;
  const aspect = dummy_model_size.width/dummy_model_element.offsetWidth *faktor;
  const near = 0.1;
  const far = 1000;
  const camera = new THREE.PerspectiveCamera( fov, aspect, near, far );
    ...
    renderer.setSize(dummy_model_element.offsetWidth, dummy_model_element.offsetWidth *faktor);
    document.getElementById("dummy_model").appendChild(renderer.domElement);

      // Robot
  const robot_arm = createRobot();
  scene.add(robot_arm);
    
    camera.position.z = 15; // move camera back so we can see the cube
    ...
    var render = function() {
      ...
      //robot_arm.rotation.x += animateSpeed;
      robot_arm.rotation.y += animateSpeed;
    };
